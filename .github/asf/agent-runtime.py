...
def run(cmd, check=True, timeout=None): return subprocess.run(cmd, text=True, capture_output=True, check=check, timeout=timeout)
...
def lease(issue_number):
    now=int(time.time()); marker=f"<!-- asf-lease:{issue_number} -->"; comments=github_json(f"https://api.github.com/repos/{os.environ['GITHUB_REPOSITORY']}/issues/{issue_number}/comments?per_page=100")
...
def build_context(title,body):
    ...
    for path in run(["git","ls-files"],check=False).stdout.splitlines():
        ...
        low=path.lower(); score=(20 if low in {"readme.md","build.gradle.kts","settings.gradle.kts"} else 0)+(8 if low.startswith("docs/") else 0)+(4 if any(t in low for t in terms) else 0)+(3 if any(x in low for x in ("app/","src/","test/",".github/")) else 0); paths.append((score,path))
...
def project_test():
    if Path("gradlew").is_file():commands=[["./gradlew","test"]]
    elif Path("pubspec.yaml").is_file():commands=[["flutter","pub","get"],["flutter","analyze"],["flutter","test"]]
...
def validate_diff(diff):
    ...
    if not diff.startswith("diff --git "):return False,"Patch must start with diff --git"
    if len(diff)>MAX_DIFF:return False,"Patch exceeds safety limit"
    lines=diff.splitlines(); starts=[i for i,l in enumerate(lines) if l.startswith("diff --git ")]
    if not starts or len(starts)>MAX_FILES:return False,"Invalid file count"
    starts.append(len(lines))
    for i in range(len(starts)-1):
        section=lines[starts[i]:starts[i+1]]; minus=next((j for j,l in enumerate(section) if l.startswith("--- ")),None); plus=next((j for j,l in enumerate(section) if l.startswith("+++ ")),None); hunk=next((j for j,l in enumerate(section) if l.startswith("@@ ")),None)
        if minus is None or plus is None or hunk is None or not(minus<plus<hunk):return False,"Each file requires --- +++ and complete @@ hunks"
        for l in section:
            if l.startswith("+++ b/") and SECRET_PATTERNS.search(l[6:]):return False,"Secret-like path is not allowed"
    return True,"ok"

def apply_diff(diff):
    diff=normalize_diff(diff); ok,msg=validate_diff(diff)
    if not ok:return False,msg
    Path("/tmp/ncm-worker.patch").write_text(diff+"\n",encoding="utf-8")
    check=run(["git","apply","--check","--recount","/tmp/ncm-worker.patch"],check=False)
    if check.returncode!=0:return False,check.stderr[-12000:]
    applied=run(["git","apply","--recount","--whitespace=fix","/tmp/ncm-worker.patch"],check=False)
    if applied.returncode!=0:return False,applied.stderr[-12000:]
    return True,"patch applied"

def prompt(issue_number,title,body,context,failure):
    repair=f"\nPrevious failure evidence:\n{failure}\n" if failure else ""
    return f"""You are the bounded autonomous Code Worker for {os.environ['GITHUB_REPOSITORY']}. Implement ONLY GitHub Issue #{issue_number}.\nTitle: {title}\nIssue body:\n{body}\n{repair}\nRepository context (secret-safe, bounded):\n{context}\nRules: preserve the canonical NetworkCenterMonitor architecture and MASTER_SPEC; no new product scope; no secrets/credentials or unrelated CI controls; prefer the smallest safe implementation and focused tests; return ONLY a complete unified git diff beginning with diff --git; every changed file needs --- +++ and complete @@ hunks; return empty output if unsafe or underspecified."""

def main():
    raw=os.getenv("NCM_ISSUE_NUMBER") or os.getenv("ASF_ISSUE_NUMBER")
    if not raw:return 2
    issue_number=int(raw); expected=os.getenv("NCM_EXPECTED_MAIN_SHA") or os.getenv("ASF_EXPECTED_MAIN_SHA") or ""; repo=os.environ["GITHUB_REPOSITORY"]
    main_sha=github_json(f"https://api.github.com/repos/{repo}/branches/main")["commit"]["sha"]
    if expected and main_sha!=expected:
        post_comment(issue_number,f"<!-- ncm-worker-result -->\nstatus=REQUEUED\nreason=MAIN_MOVED\nexpected={expected}\nactual={main_sha}");return 7
    issue=github_json(f"https://api.github.com/repos/{repo}/issues/{issue_number}"); labels={x.get("name") for x in issue.get("labels",[])}; body=issue.get("body","") or ""
    if not ("NCM-AUTO: TRUE" in body or "ncm-auto" in labels):raise RuntimeError("Issue is not opted into NCM autonomous execution")
    lease(issue_number); title=issue.get("title",""); context=build_context(title,body); deadline=time.monotonic()+PROVIDER_BUDGET; failure=""
    for attempt in range(1,MAX_FIX_ATTEMPTS+1):
        ...
        if not ok:run(["git","reset","--hard","HEAD"],check=False);failure=f"Patch rejected: {msg}";continue
        passed,evidence=project_test()
        if passed:print("NCM_CODE_WORKER_PASS");return 0
